...
movie.getListEpisode = async (req, res, next) => {
  let movie_id = req.params.movie_id;
  try {
    next({
      data: await MovieModule.get_list_episode(movie_id),
    });
  } catch (e) {
...
movie.getComingSoon = async (req, res, next) => {
  try {
    next({
      data: await MovieModule.get_coming_soon(),
    });
  } catch (e) {
    next(new Err(e.message, 500, e.constraint));
  }
...
movie.getRelateMovie = async (req, res, next) => {
  try {
    let movie = await MovieModule.get_relate_movie(req.params.movie_id);
...
movie.getFullMovie = async (req, res, next) => {
  try {
    next({
      data: await MovieModule.get_full_series(),
    });
  } catch (e) {
    next(new Err(e.message, 500, e.constraint));
  }
...
// movie.getFullSeries = async (req, res, next) => {
//   try {
//     let movie = await MovieModule.get_full_series();
//     if (!movie) return next(new Err(message.movie.not_found, 404));
//     movie.episodes = await MovieModule.get_list_episode(req.params.movie_id);
//     next({
//       data: [movie],
//     });
//   } catch (e) {
...
movie.getDetailMovie = async (req, res, next) => {
  try {
    ...
    movie.episodes = await EpisodeModule.get_all(req.params.movie_id);
    movie.genres = await MovieModule.get_list_genre_movie(req.params.movie_id);
