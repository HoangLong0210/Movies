...
viewer.resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!account_id) {
      return next(new Err(message.viewer.not_found));
    }
    if (await ViewerModule.update_password({ account_id, password })) {
      return next({
        data: [{ updated: true }],
        message: message.viewer.password_updated,
      });
    } else return next(new Err(message.viewer.something_wrong));
  } catch (e) {
    next(new Err(e.message, 500, e.constraint));
  }
};

viewer.updateInfo = async (req, res, next) => {
  let { name, avatar, email, address } = req.body;
  let { viewer_id } = req.viewer;
};
