...
movie.updateBook = async (req, res, next) => {
  ...
  try {
    if (!(await BookModule.get(req.params.endpoint)))
      return next(new Err(message.book.not_found, 404));

    if (req.files["thumb"]) {
      if (!req.files["thumb"][0]["mimetype"].includes("image"))
        return next(new Err(message.book.thumb_invalid, 400));
      else
        book.thumb = await FileModule.upload_single(
          req.files["thumb"][0],
          "book/" + book.endpoint + "/",
          "thumb"
        );
    ...
    if (req.files["theme"]) {
      if (!req.files["theme"][0]["mimetype"].includes("image"))
        return next(new Err(message.book.theme_invalid, 400));
      else
        book.theme = await FileModule.upload_single(
          req.files["theme"][0],
          "book/" + book.endpoint + "/",
          "theme"
        );
    ...
    book.follow = (
      await BookModule.get_user_follow(req.params.endpoint)
    ).length;
    book.genres = [];
