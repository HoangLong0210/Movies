...
movie.updateBook = async (req, res, next) => {
  ...
  try {
    ...
    movie = await MovieModule.update_info(movie, req.params.endpoint);
    movie.view = (await MovieModule.get_view_all(req.params.endpoint)).length;
    movie.follow = (
      await MovieModule.get_user_follow(req.params.endpoint)
    ).length;
    movie.genres = [];
