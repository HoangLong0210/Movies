...
movie.addMovie = async (req, res, next) => {
  ...
  try {
    ...
    else if (!movie.type) return next(new Err(message.movie.missing_type, 400));
    else if (!req.files["thumb"])
      return next(new Err(message.movie.missing_thumb, 400));
    else if (!req.files["theme"])
      return next(new Err(message.movie.missing_theme, 400));
    else if (!req.files["thumb"][0]["mimetype"].includes("image"))
      return next(new Err(message.movie.thumb_invalid, 400));
    else if (!req.files["theme"][0]["mimetype"].includes("image"))
      return next(new Err(message.movie.theme_invalid, 400));
    ...
    movie.theme = await FileModule.upload_single(
      req.files["theme"][0],
      "movie/" + movie.endpoint + "/",
      "theme"
    );
    ...
    if (genres) {
      ...
      for (bg of moviegenre) {
        movie.genres.push(await GenreModule.get(bg.genre_endpoint));
      }
    }

    await movieModule.add_view(
      movie.endpoint,
