...
movie.get_relate_movie = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id];
    let query = `select * from "Movie", (select movie_id, count(movie_id) as count from "MovieGenres" where genre_id in(select genre_id from "MovieGenres" where movie_id = $1) and movie_id <> $1 group by movie_id order by count desc limit 20) as g where g.movie_id = movie_id`;
...
'slug', btrim(slug), 'title', btrim(title), 'description', btrim(description))) genres from "Genre" g, (select * from "MovieGenres" where movie_id = $1) mg where g.genre_id = mg.genre_id) g, (select count(viewer_id) follow from "MovieFollows" where movie_id = $1) f`;
