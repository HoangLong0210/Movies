...
book.get_relate_book = (endpoint) => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie", (select movie_id, count(movie_id) as count from "MovieGenres" where genre_id in
(select genre_id from "MovieGenres" where movie_id = $1) and movie_id <> $1 group by movie_id order by count desc limit 10) as g where g.movie_id = "Movie".movie_id`;
