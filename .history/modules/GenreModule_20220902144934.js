...
genre.get_all_movie = (genre_id) => {
  return new Promise((resolve, reject) => {
    let params = [genre_id];
    let query = ` select * from "Movie" m inner join "MovieGenres" MG on m.movie_id = MG.movie_id inner join "Genre" G on MG.genre_id = G.genre_id where  G.genre_id= $1 and m.typemovie='Phim lẻ' `;
