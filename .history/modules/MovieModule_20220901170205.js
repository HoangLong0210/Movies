...
movie.get_detail = (movie_id) => {
  return new Promise((resolve, reject) => {
    ...
    (select json_agg(jsonb_build_object('movie_id', movie_id, 'slug', btrim(slug), 'title', btrim(title), 'description', btrim(description))) genres from "Genre" g, (select * from "MovieGenres" where movie_id = $1) mg where g.genre_id = mg.genre_id) g, (select count(viewer_id) follow from "MovieFollows" where movie_id = $1) f, (select json_agg(jsonb_build_object('person_id' person_id, 'name', btrim(name))) person from "FamousPerson" fp,(select * from "Mission" where movie_id = $1) mi where fp.person_id = mi.person_id) fp`;
