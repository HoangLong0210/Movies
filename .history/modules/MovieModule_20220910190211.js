...
movie.get_relate_movie = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id];
    let query = `select * from "Movie" M, (select movie_id, count(movie_id) as count from "MovieGenres" where genre_id in (select genre_id from "MovieGenres" where movie_id = $1) and movie_id <> $1 group by movie_id order by count desc limit 20) as G where G.movie_id = M.movie_id`;
...
movie.get_list_episode = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id];
    let query = `select * from "Episode" E inner join "Movie" M on E.movie_id = M.movie_id where E.movie_id= $1 and typemovie='Phim bộ'`;
...
movie.get_coming_soon = () => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie"  where status_movie = 'Sắp công chiếu' order by movie_id  limit 20`;
...
movie.get_full = () => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie" where  typemovie='Phim lẻ' order by movie_id`;
...
movie.get_full_series = () => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie" where  typemovie='Phim bộ' order by movie_id`;
...
movie.get_list_genre_movie = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id];
    let query = `select G.genre_id, G.slug, G.title, G.description from "Genre" G inner join "MovieGenres" MG on G.genre_id = MG.genre_id inner join "Movie" M on MG.movie_id = M.movie_id where M.movie_id= $1`;
