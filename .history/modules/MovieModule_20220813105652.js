...
movie.get_top_view = () => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie" order by view_number desc limit 10`;
...
movie.get_top_follow = () => {
  return new Promise((resolve, reject) => {
    let query = `select m.* from (select movie_id, count(viewer_id) as count from "MovieFollows" group by movie_id limit 10) as v, "Movie" m where v.movie_id = m.movie_id order by count desc`;
...
movie.get_relate_book = (movie_id) => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Movie", (select movie_id, count(movie_id) as count from "MovieGenres" where genre_id in(select genre_id from "MovieGenres" where movie_id = $1) and movie_id <> $1 group by movie_id order by count desc limit 10) as g where g.movie_id = movie_id`;
...
movie.get_detail = (movie_id) => {
  return new Promise((resolve, reject) => {
    let query = `select * from (select * from "Movie" m where movie_id = $1 limit 1) b,
    (select json_agg(jsonb_build_object('movie_id', movie_id,
                                        'title', btrim(title),
                                        'description', btrim(description))) genres
    from "Genre" g,
         (select * from "MovieGenres" where movie_id = $1) mg
    where g.genre_id = mg.genre_id) g,
    (select count(viewer_id) follow from "MovieFollows" where movie_id = $1) f`;
...
movie.get_viewer_follow = (movie_id) => {
  return new Promise((resolve, reject) => {
    let query = `select viewer_id, account_id, avatar, email from "Viewers" where viewer_id in (select viewer_id from "MovieFollows" where movie_id = $1)`;
...
movie.delete_all_genres = (genre_id) => {
  return new Promise((resolve, reject) => {
    let query = 'delete from "MovieGenres" where movie_id = $1';
