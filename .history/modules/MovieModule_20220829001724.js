...
movie.filter_with_genres = (filter, page) => {
  return new Promise((resolve, reject) => {
    ...
    let query = `select m.* from "Movie" m, (select json_agg(jsonb_build_object('movie_id', movie_id,
    'slug', btrim(slug), 'title', btrim(title), 'description', btrim(description))) genres from "Genre" g, (select * from "MovieGenres" where movie_id = $1) mg where g.genre_id = mg.genre_id) g, (select movie_id count(movie_id) as count from "MovieGenres" where genre_id = any($1:: []) group by movie_id) as mg where m.movie_id = mg.movie_id`;
    if (filter.title) {
