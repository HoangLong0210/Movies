...
genre.get_list_movie = (genre_id) => {
  return new Promise((resolve, reject) => {
    let params = [genre_id];
    let query = `select m.movie_id, m.slug, m.title, m.titleenglish, m.poster, m.description, m.releasemovie, m.nation, m.typeservice, m.url_trailer, m.rating, m.rate_count, m.search_number, m.view_number, m.typemovie, m.status_movie, G.genre_id, G.slug, G.title, G.description from "Movie" m inner join "MovieGenres" MG on m.movie_id = MG.movie_id inner join "Genre" G on MG.genre_id = G.genre_id where G.genre_id= $1 `;
