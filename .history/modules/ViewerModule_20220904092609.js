...
viewer.find_viewer_by_account = (account_id) => {
  return new Promise((resolve, reject) => {
    ...
    let query = `select v.viewer_id, name, avatar, phone, email, address, gender, a.role_id, a.status from "Viewers" v, "Account" a where u.account_id=a.account_id and a.account_id= $1`;
...
viewer.find_account_by_viewer = ({ username }) => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Account" where username=$1 and active<>-1`;
...
viewer.is_valid_access_token = ({ account_id, access_token }) => {
  return new Promise((resolve, reject) => {
    let query = `select 1 as exist from "Account" where account_id=$1 and  $2 = ANY (access_token::varchar[] as exist from "Account" where account_id=$1 and active=true and  $2 = ANY (access_tokens::varchar[])`;
...
viewer.create_account = ({ username, password }) => {
  return new Promise((resolve, reject) => {
    ...
    let params = [username, password, "Khách hàng"];
...
viewer.login = ({ username, password }) => {
  return new Promise((resolve, reject) => {
    let query = `select * from "Account" a where a.username=$1 and a.status<>-1`;
    let params = [username];
