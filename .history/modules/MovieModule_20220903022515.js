...
movie.get_detail = (movie_id) => {
  return new Promise((resolve, reject) => {
    ...
    let query = `select * from (select * from "Movie" m where movie_id = $1 limit 1) m, (select G.genre_id, G.title, G.slug, G.description from "Genre" G inner join "MovieGenres" MG on G.genre_id = MG.genre_id inner join "Movie" M on MG.movie_id = M.movie_id where M.movie_id= $1) f`;
