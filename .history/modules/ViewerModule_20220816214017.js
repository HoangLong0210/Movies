...
viewer.is_valid_access_token = ({ account_id, access_token }) => {
  return new Promise((resolve, reject) => {
    let query = `select 1select 1 as exist from "Account" where account_id=$1 and  $2 = ANY (access_token::varchar[] as exist from "Account" where account_id=$1 and active=true and  $2 = ANY (access_tokens::varchar[])`;
