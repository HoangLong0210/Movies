...
movie.get_relate_movie = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id];
    let query = `select * from "Movie" M, (select movie_id, count(movie_id) as count from "MovieGenres" where genre_id in (select genre_id from "MovieGenres" where movie_id = $1) and movie_id <> $1 group by movie_id order by count desc limit 20) as g where M.movie_id = movie_id`;
