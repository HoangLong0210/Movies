...
movie.get_detail = (movie_id) => {
  return new Promise((resolve, reject) => {
    let params = [movie_id, genre_id];
    let query = `select * from (select * from "Movie" m where movie_id = $1 limit 1) m,
    (select json_agg(jsonb_build_object('genre_id', genre_id, 'slug', btrim(slug), 'title', btrim(title), 'description', btrim(description))) genres from "Genre" g, (select * from "MovieGenres" where movie_id = $1) mg where g.genre_id = mg.genre_id) g, (select count(viewer_id) follow from "MovieFollows" where movie_id = $1) f`;
